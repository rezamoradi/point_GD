...
def fps(points: torch.Tensor, ratio: float = None, num_pnts: int = None, random_start: bool = False) -> torch.Tensor:
    """
    Wrapper for PyTorch3D's farthest point sampling implementation.
    
    Args:
        points (torch.Tensor): Input points with shape (N, D) where N is the number
            of points and D is the dimension
        ratio (float, optional): Ratio of points to sample. Either ratio or num_pnts must be specified
        num_pnts (int, optional): Number of points to sample. Either ratio or num_pnts must be specified
        random_start (bool): Whether to use random initialization. Default: False
    ...
    # Calculate number of samples if ratio is provided
    if ratio is not None:
        num_pnts = int(ratio * points.shape[1])
    ...
    # Return indices for the first (and only) batch
    return indices[0]
